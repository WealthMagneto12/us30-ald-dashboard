    ...
    def backtest(self):
        ...
        for idx, row in self.df.iterrows():
            if row['ALD_Signal'] in ['Long', 'Short']:
                ...
                if row['ALD_Signal'] == 'Long':
                    pnl = (tp - row['Close']) * position_size if row['High'] >= tp else (sl - row['Close']) * position_size if row['Low'] <= sl else 0
                else:
                    pnl = (row['Close'] - tp) * position_size if row['Low'] <= tp else (row['Close'] - sl) * position_size if row['High'] >= sl else 0

                outcome = 'TP Hit' if pnl > 0 else 'SL Hit' if pnl < 0 else 'No Hit'
